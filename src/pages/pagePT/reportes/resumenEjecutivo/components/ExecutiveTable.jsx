...
export default function ExecutiveTable({
  ...
  const getDetalleServicios = (v) => v?.detalle_ventaservicios || v?.detalle_ventaServicios || v?.detalle_servicios || v?.detalle_venta_servicios || [];
  ...
  function computeMetricsForMonth(anio, mesNombre) {
    ...
    for (const v of ventas) {
      ...
      const pagos = Array.isArray(v?.detalleVenta_pagoVenta) ? v.detalleVenta_pagoVenta : (Array.isArray(v?.detalle_pagoVenta) ? v.detalle_pagoVenta : []);
      ...
      const rawOrigin = v?.id_origen ?? v?.parametro_origen?.id_param ?? v?.origen ?? v?.source ?? v?.canal ?? v?.parametro_origen?.label_param;
    ...
    if (metaServTotalFull > 0) {
      ...
      addTo(byOriginFull, "instagram", "INSTAGRAM", metaServTotalFull * igShare, metaServCantCut * igShare);
    }
    ...
    const byOriginCli = Object.fromEntries(Object.entries(byOriginCliSet).map(([k, s]) => [k, (s?.size || 0)]));
    const byOriginCliFull = Object.fromEntries(Object.entries(byOriginCliSetFull).map(([k, s]) => [k, (s?.size || 0)]));
      ...
      <table style={sTable}>
        ...
        <tfoot>
          ...
          <tr>
            ...
            {perMonth.map((m, idx) => (
              <td key={`footer-month-${idx}`} style={{ ...sCellBold, background: gold, color: "#000", textAlign: "center" }}>{m.label}</td>
            ))}
          </tr>
        </tfoot>
      </table>
