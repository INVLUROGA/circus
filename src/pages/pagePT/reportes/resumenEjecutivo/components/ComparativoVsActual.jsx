...
function SectionHeader({ name, columns }) {
  return (
    <tr>
      <th className="bg-yellow-400 text-black border border-black px-3 text-left uppercase font-extrabold fs-3">{name}   </th>
      {columns.map((c) => (
        <th key={c.key} className="bg-yellow-400 text-black border border-black px-3 text-left uppercase font-extrabold fs-3">
          <div>
...
export const monthKey = (iso) => MONTHS[ dayjs.utc(iso).month() ];
export const dayOfMonth = (iso) => dayjs.utc(iso).date();
const montoServ = (det) => {
...
export function buildComparativoFromVentas(ventas = [], { columns = [], cutDay = null, referenceMonth = null } = {}) {
  ...
  ventas.forEach((v) => {
    ...
    const srv = Array.isArray(v?.detalle_ventaservicios) ? v.detalle_ventaservicios : [];
