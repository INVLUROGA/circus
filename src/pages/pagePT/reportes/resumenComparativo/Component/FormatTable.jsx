...
export const FormatTable = ({ data=[], tableColor, propColor }) => {
  ...
  return (
    <Table className="table-centered mb-0" striped responsive>
      <thead className={tableColor?tableColor:'bg-primary'}>
        <tr>
          {headers?.map((header, index) => {
            const isSortable = data[0][index]?.isSortable;
            return (
              <th className={`text-white fs-3 pl-5`} key={index} onClick={() => isSortable && handleSort(header)} style={{ cursor: isSortable ? "pointer" : "default" }}>
        ...
        {sortedData.map((row, rowIndex) => (
          <tr key={rowIndex}>
            {row?.map((col, colIndex) => (
              ...
              <li className={`d-flex flex-row justify-content-between p-2  ${(`${col.value}`.split(' ')[1]==='PM'?'bg-primary text-white':`${col.isPropiedad?`${propColor?propColor:'text-primary'}`:'text-black'}`)}`}>
                
                                    <h2 style={{fontSize: '25px'}} className={`fw-bold ml-3 ${col.isPropiedad? 'word-break-keep-d-inline-flex':''}`}><span className={`'mr-4 ${propColor?propColor:'text-black'}'`}>{col.isIndexado? <span className='mr-2'>{`${rowIndex+1}) `} </span>:''}</span> {col.HTML ?? col.value}</h2>
              </li>
            </td>
            ))}
          </tr>
      ...
      <tfoot>
        <tr className={tableColor?tableColor:'bg-primary'}>
